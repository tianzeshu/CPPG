...
class PromptBartDecoder(nn.Module):
    ...
    def forward(self, tgt_tokens, prompt_state):
        ...
        word_scores = word_scores.masked_fill(mask, -1e4)
...
class PromptBartModel(nn.Module):
    def __init__(self, tokenizer, label_ids, args):
        ...
        self.worker_embed = nn.Embedding(48, 1024)

        self.worker_num = 48  # number of workers
        self.training_worker_ids = list(set(i for i in range(1, self.worker_num)))  # assume worker_ids is the list of worker ids in your training data
        self.weights = {worker_id: 1.0 / len(self.training_worker_ids) for worker_id in self.training_worker_ids}

        # 对于每个标注者创建其私有的prompt参数，有多少个worker就有多少个私有prompt
        self.encoder_prompt_embeds = nn.ModuleList([nn.Embedding(self.prompt_len, bart_config.d_model) for _ in range(self.worker_num)])
        self.decoder_prompt_embeds = nn.ModuleList([nn.Embedding(self.prompt_len, bart_config.d_model) for _ in range(self.worker_num)])
        self.prompt_cross_embeds = nn.ModuleList([nn.Embedding(self.prompt_len, bart_config.d_model) for _ in range(self.worker_num)])
        ...
        # 对所有标注者都生效的公共prompt部分
        self.common_encoder_prompt_embed = nn.Embedding(self.prompt_len, bart_config.d_model)
        self.common_decoder_prompt_embed = nn.Embedding(self.prompt_len, bart_config.d_model)
        self.common_prompt_cross_embed = nn.Embedding(self.prompt_len, bart_config.d_model)

        self.common_encoder_mlp = nn.Sequential(
            nn.Linear(bart_config.d_model, self.prompt_dim),
            nn.Tanh(),
            nn.Linear(self.prompt_dim, bart_config.decoder_layers * 2 * bart_config.d_model))
        self.common_decoder_mlp = nn.Sequential(
            nn.Linear(bart_config.d_model, self.prompt_dim),
            nn.Tanh(),
            nn.Linear(self.prompt_dim, bart_config.decoder_layers * 2 * bart_config.d_model))
        self.common_cross_mlp = nn.Sequential(
            nn.Linear(bart_config.d_model, self.prompt_dim),
            nn.Tanh(),
            nn.Linear(self.prompt_dim, bart_config.decoder_layers * 2 * bart_config.d_model))
    ...
    def forward(self, src_tokens, tgt_tokens, src_seq_len, first, worker):
        prompt_state = self.generator(src_tokens, src_seq_len, first, worker)
        
        decoder_outputs, prompt_state = self.prompt_decoder(tgt_tokens, prompt_state)
        return decoder_outputs


    def generator(self, src_tokens, src_seq_len, first, worker):
        batch_size = src_tokens.size(0)
        past_key_values = self.get_prompt(batch_size, worker) if self.use_prompt else None
    ...
    def get_prompt(self, batch_size, worker):

        # select private parameters for the current worker
        # 这里的worker必须是单个的id，使用batch_size=1进行训练即可，速度慢点但是可以训练，思路简单
        # 测试的时候使用加权平均的私有参数
        if worker is None:
            worker = -1
        else:
        ...
        # check if worker id is in the training data
        if worker in self.training_worker_ids:
            # select private parameters for the current worker
            encoder_prompt_embed = self.encoder_prompt_embeds[worker](input_tokens)
            decoder_prompt_embed = self.decoder_prompt_embeds[worker](input_tokens)
            prompt_cross_embed = self.prompt_cross_embeds[worker](input_tokens)
        else:
            # calculate the weighted average of private parameters for unknown worker
            encoder_prompt_embed = sum(self.encoder_prompt_embeds[i](input_tokens) * w for i, w in self.weights.items())
            decoder_prompt_embed = sum(self.decoder_prompt_embeds[i](input_tokens) * w for i, w in self.weights.items())
            prompt_cross_embed = sum(self.prompt_cross_embeds[i](input_tokens) * w for i, w in self.weights.items())

        # generate prompt parameters with private and common parameters
        encoder_embed = encoder_prompt_embed + self.common_encoder_prompt_embed(input_tokens)
        decoder_embed = decoder_prompt_embed + self.common_decoder_prompt_embed(input_tokens)
        cross_embed = prompt_cross_embed + self.common_prompt_cross_embed(input_tokens)

        # process past_key_values, past_key_values2, and past_key_values_enc as before ...
        if worker in self.training_worker_ids:
            past_key_values = self.encoder_mlps[worker](encoder_embed) + self.common_encoder_mlp(encoder_embed)
            past_key_values2 = self.decoder_mlps[worker](decoder_embed) + self.common_decoder_mlp(decoder_embed)
            past_key_values_enc = self.cross_mlps[worker](cross_embed) + self.common_cross_mlp(cross_embed)
        else:
            past_key_values = sum(self.encoder_mlps[i](encoder_embed) * w for i, w in self.weights.items()) + self.common_encoder_mlp(encoder_embed)
            past_key_values2 = sum(self.decoder_mlps[i](decoder_embed) * w for i, w in self.weights.items()) + self.common_decoder_mlp(decoder_embed)
            past_key_values_enc = sum(self.cross_mlps[i](cross_embed) * w for i, w in self.weights.items()) + self.common_cross_mlp(cross_embed)

            # past_key_values = self.common_encoder_mlp(encoder_embed)
            # past_key_values2 = self.common_decoder_mlp(decoder_embed)
            # past_key_values_enc = self.common_cross_mlp(cross_embed)
    ...
    #     # 这里的worker必须是单个的id，使用batch_size=1进行训练即可，速度慢点但是可以训练，思路简单
    #     # 测试的时候
    #     encoder_prompt_embed = self.encoder_prompt_embeds[worker]
    #     decoder_prompt_embed = self.decoder_prompt_embeds[worker]
    #     prompt_cross_embed = self.prompt_cross_embeds[worker]
    #     encoder_mlp = self.encoder_mlps[worker]
    #     decoder_mlp = self.decoder_mlps[worker]
    #     cross_mlp = self.cross_mlps[worker]
    #
    #     # generate prompt parameters with private and common parameters
    #     encoder_embed = encoder_prompt_embed(input_tokens) + self.common_encoder_prompt_embed(input_tokens)
    #     decoder_embed = decoder_prompt_embed(input_tokens) + self.common_decoder_prompt_embed(input_tokens)
    #     cross_embed = prompt_cross_embed(input_tokens) + self.common_prompt_cross_embed(input_tokens)
    #
    #     past_key_values = encoder_mlp(encoder_embed) + self.common_encoder_mlp(encoder_embed)
    #     past_key_values2 = decoder_mlp(decoder_embed) + self.common_decoder_mlp(decoder_embed)
    #     past_key_values_enc = cross_mlp(cross_embed) + self.common_cross_mlp(cross_embed)
    #
    #     bsz, seqlen, _ = past_key_values.shape
    #     past_key_values = past_key_values.view(bsz, seqlen, self.bart_config.decoder_layers * 2,
    #                                             self.bart_config.decoder_attention_heads, self.bart_config.d_model // self.bart_config.decoder_attention_heads)
...
class PromptGeneratorModel(nn.Module):
    ...
    def forward(self, src_tokens, tgt_tokens, src_seq_len=None, tgt_seq_len=None, first=None, worker=None):
        """
        ...
        :return:
        """
        return self.prompt_model(src_tokens, tgt_tokens, src_seq_len, first, worker)
    ...
    def predict(self, src_tokens, src_seq_len=None, first=None):
        ...
        :return:
        """
        prompt_state = self.prompt_model.generator(src_tokens, src_seq_len, first, worker=None)    # encoder output
...
def _beam_search_generate(decoder: PromptBartDecoder, tokens=None, state=None, max_length=20, max_len_a=0.0, num_beams=4,
    ...
    while cur_len < real_max_length:
        ...
        if _eos_token_id!=-1:
            ...
            scores[:, _eos_token_id] = torch.where(max_len_eos_mask, eos_scores+1e8, eos_scores)
