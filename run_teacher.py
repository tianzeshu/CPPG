...
from module.teacher_train import Trainer
...
def main(cfg):
    ...
    prompt_model = TeacherPromptBartModel(tokenizer=process.tokenizer, label_ids=label_ids, args=cfg)
    model = TeacherPromptGeneratorModel(prompt_model=prompt_model, bos_token_id=0,
                                        eos_token_id=1,
    ...
    for worker_id in range(1, 48):
        ...
        trainer = Trainer(train_data=train_dataloader, dev_data=dev_dataloader, test_data=None, model=model, args=cfg, logger=logger, loss=loss,
                          metrics=metrics, device=device, worker_id=worker_id)
        trainer.train()
