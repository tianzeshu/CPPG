...
class Trainer(object):
    def __init__(self, train_data=None, dev_data=None, test_data=None, model=None, process=None, args=None, logger=None, loss=None, metrics=None, writer=None, device=None, use_multi_gpu=False, teacher_model_list=None) -> None:
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            # 遍历47个worker的数据
            for dataloader_index, data in enumerate(self.train_data):
                data_bar = tqdm(data)
                data_bar.set_description(f"Annotator {dataloader_index}")
            ...
            if epoch >= self.eval_begin_epoch:
                if (self.use_multi_gpu and int(os.environ.get("LOCAL_RANK", -1)) == 0) or not self.use_multi_gpu:
    ...
    def _step(self, batch, mode="train"):
        if mode=="dev": # dev: compute metric
            ...
            src_tokens, tgt_tokens, src_seq_len, tgt_seq_len, first, target_span, worker = batch
            pred = self.model(src_tokens, tgt_tokens, src_seq_len, first=first, worker=worker)
            soft_pred = self.teacher_model_list[worker[0]](src_tokens, tgt_tokens, src_seq_len, first=first)
            # kl_loss = KL(pred, soft_pred)
